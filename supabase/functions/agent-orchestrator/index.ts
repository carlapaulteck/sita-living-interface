...
async function executeWorkflow(
  ...
  for (let i = 0; i < steps.length; i++) {
    ...
    // Build input from context and previous results
    let input = { ...context };
    if (step.input_mapping) {
      for (const [key, source] of Object.entries(step.input_mapping)) {
        const [stepIndex, field] = source.split(".");
        if (stepIndex === "context") {
          input[key] = context[field];
        } else {
          const sourceResult = results[parseInt(stepIndex)];
          if (sourceResult) {
            input[key] = (sourceResult as Record<string, unknown>)[field];
          }
        }
      }
    }
    ...
    // Record step in workflow coordination
    if (workflowId) {
      await supabase.from("agent_workflow_steps").insert({
        workflow_id: workflowId,
        workflow_run_id: workflowRunId,
  ...
  if (workflowId) {
    await supabase.rpc("increment_workflow_runs", { workflow_id: workflowId });
  }
