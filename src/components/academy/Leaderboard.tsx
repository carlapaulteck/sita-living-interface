...
export const Leaderboard = () => {
  ...
  const { profiles, memberPoints, leaderboard, user } = useAcademy();
    ...
    <div className="space-y-6">
            ...
            <div className="flex items-center gap-4 mb-6">
              ...
              <div className="flex-1">
                <div className="flex items-center gap-2">
                  <h3 className="text-lg font-bold text-foreground">{profiles.find(p => p.user_id === user?.id)?.display_name || 'You'}</h3>
                  {(() => {
                    const config = getLevelConfig(memberPoints.current_level || 1);
                    const Icon = config.icon;
                    return <Badge className={cn("gap-1", config.color, "bg-current/10 border-current/30")}><Icon className="w-3 h-3" />{config.name}</Badge>;
                  })()}
                </div>
                <div className="flex items-center gap-4 text-sm text-muted-foreground mt-1">
                  <span className="flex items-center gap-1"><Trophy className="w-4 h-4 text-primary" />Rank #{currentUserRank || '-'}</span>
                  <span className="flex items-center gap-1"><Zap className="w-4 h-4 text-secondary" />{memberPoints.total_points || 0} XP</span>
                  <span className="flex items-center gap-1"><Flame className="w-4 h-4 text-orange-400" />{memberPoints.streak_days || 0} day streak</span>
                </div>
              </div>
            </div>
      ...
      <Tabs value={timeFilter} onValueChange={setTimeFilter}>
        <TabsList className="w-full bg-card/50 border border-border/50">
          {timeFilters.map((filter) => <TabsTrigger key={filter.id} value={filter.id} className="flex-1">{filter.label}</TabsTrigger>)}
        </TabsList>
          ...
          <GlassCard hover={false}>
            <div className="space-y-2">
              {leaderboard.map((member, index) => {
                ...
                return (
                  <motion.div key={member.user_id} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: index * 0.03 }}
                    className={cn("flex items-center gap-4 p-3 rounded-xl transition-all", isCurrentUser ? "bg-primary/10 border border-primary/30" : "hover:bg-muted/30")}
                  >
                    <div className={cn("w-8 h-8 rounded-lg flex items-center justify-center font-bold text-sm",
                      index < 3 ? index === 0 ? "bg-yellow-400/20 text-yellow-400" : index === 1 ? "bg-slate-300/20 text-slate-300" : "bg-amber-600/20 text-amber-600" : "bg-muted text-muted-foreground"
                    )}>{index + 1}</div>
                    <Avatar className="w-10 h-10 border border-border">
                      <AvatarImage src={profile?.avatar_url || undefined} />
                      <AvatarFallback className="bg-gradient-to-br from-primary/20 to-secondary/20">{profile?.display_name?.[0] || 'U'}</AvatarFallback>
                    </Avatar>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        <span className={cn("font-medium truncate", isCurrentUser ? "text-primary" : "text-foreground")}>{profile?.display_name || 'Member'}{isCurrentUser && " (You)"}</span>
                        <Badge variant="outline" className={cn("text-xs", config.color)}><Icon className="w-3 h-3 mr-1" />{config.name}</Badge>
                      </div>
                      <p className="text-xs text-muted-foreground">Level {member.current_level || 1} • {member.streak_days || 0} day streak</p>
                    </div>
